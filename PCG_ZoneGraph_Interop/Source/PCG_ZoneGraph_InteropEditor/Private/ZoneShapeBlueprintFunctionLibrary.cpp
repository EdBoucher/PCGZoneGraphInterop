...
void UZoneShapeBlueprintFunctionLibrary::PGLSetPointPositionRotation(
    ...
    const FVector& TargetPointWorldNormal)
{
    ...
    //FVector Normal = SourceTransform.TransformVector(TargetPointWorldNormal);
...
bool ComparePoints(const FVector& P1, const FVector& P2)
{
 ...
 void UZoneShapeBlueprintFunctionLibrary::PGLSortPolygonPointsCounterclockwise(UZoneShapeComponent* PolygonShapeComp)
{
...
void UZoneShapeBlueprintFunctionLibrary::PGLSortPCGPointsCounterclockwise(TArray<FPCGPoint>& PCGPoints)
{
	...
	Center /= PCGPoints.Num();

	PCGPoints.Sort([Center](const FPCGPoint& Point1, const FPCGPoint& Point2) {
		return ComparePoints(Point1.Transform.GetLocation() - Center, Point2.Transform.GetLocation() - Center);
	});
}


void UZoneShapeBlueprintFunctionLibrary::PGLCreateIntersectionForSplineShape(
    ...
    bool DestroyCoveredShape /*= true*/)
{
	...
	if (ShapeComponents.Num() == 1)
	{
		...
		PGLSetPointPositionRotation(Points[0], IntersectionTransform, Point0WorldPosition, Point0WorldNormal);
		...
		PGLSetPointPositionRotation(Points[1], IntersectionTransform, Point1WorldPosition, Point1WorldNormal);
	...
	else if (ShapeComponents.Num() == 2)
	{
		...
		if (ShapeComponents[0])
		{
			...
			PGLSetPointPositionRotation(Points[IntersectionPointIndex], IntersectionTransform, PointWorldPosition , PointWorldNormal);
		...
		PGLSetPointPositionRotation(Points[IntersectionPointIndex], IntersectionTransform, Intersection - Offset, WorldNormal);
		PGLSetPointPositionRotation(TargetPoints[PointBIndex], TargetShapeCompTransform,Intersection - Offset, WorldNormal);

		PGLSortPolygonPointsCounterclockwise(IntersectionShapeComponent);
...
TArray<UZoneShapeComponent*> UZoneShapeBlueprintFunctionLibrary::PGLBreakZoneShapeAtLocation(
    UZoneShapeComponent* ShapeComp,
